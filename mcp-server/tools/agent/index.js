...
module.exports = {
  async run(input) {
    const {
      ...
      companyName = "Tealium Demo Co",
      agentMode = "customer"   // expected values: "customer" | "cs_copilot"
    } = input;

    console.log("🧠 /agent mode:", agentMode, "| companyName:", companyName);
    ...
    if (messageInsights.intent === "evaluate_plate" || messageInsights.topic === "number plate") {
      ...
      if (plateText) {
        ...
        await callTool("send-plate-score-to-tealium", {
          ...
          collectApiUrl: collectApiUrl,  // <- from UI
          traceId: traceId               // <- from UI
        });
    ...
    const promptToolName =
      agentMode === "cs_copilot"
        ? "construct-prompt-cs-copilot"
        : "construct-prompt-end-customer";

    console.log("🧩 Using prompt tool:", promptToolName);

    const constructedPrompt = await callTool(promptToolName, {
    ...
    lastToolResults[promptToolName] = constructedPrompt;
